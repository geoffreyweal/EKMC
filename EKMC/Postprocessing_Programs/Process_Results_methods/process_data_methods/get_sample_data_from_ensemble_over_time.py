...
def get_sample_data_from_ensemble_over_time(data_over_time, centre_of_masses, unit_cell_matrix, cpu_count=1):
    """
    ...
    centre_of_masses : list of numpy.array
    ...
    data = process_map(get_displacement_vector_from_initial_position, get_inputs(data_over_time, centre_of_masses, unit_cell_matrix), max_workers=cpu_count, unit=' KMC Sim', total=len(data_over_time))
...
def get_inputs(data_over_time, centre_of_masses, unit_cell_matrix):
    """
    This generator is designed to give thr data for each KMC simulation in data_over_time, along with centre_of_masses, unit_cell_matrix and initial_position
    ...
    for datum_at_time in data_over_time:
        yield (datum_at_time, centre_of_masses, unit_cell_matrix)
...
def get_displacement_vector_from_initial_position(input_datum):
    ...
    data_over_time, centre_of_masses, unit_cell_matrix = input_datum
    ...
    initial_position = get_position(initial_molecule, initial_cell_point, centre_of_masses, unit_cell_matrix)
    ...
    for molecule, cell_point, energy in data_over_time:

        # Fourth, obtain the position of the exciton in the crystal.
        position_at_time = get_position(molecule, cell_point, centre_of_masses, unit_cell_matrix)
        positions_at_time.append(position_at_time)
...
def get_position(molecule, cell_point, centre_of_masses, unit_cell_matrix):
    """
    ...
    position = np.matmul(cell_point,unit_cell_matrix) + centre_of_masses[molecule]
