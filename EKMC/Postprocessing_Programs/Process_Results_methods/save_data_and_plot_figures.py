...
def save_data_and_plot_figures(data_foldername, path, times, positions_at_time, average_displacements_from_initial_position_over_time, average_displacements_squared_from_initial_position_over_time, average_energies_over_time, diffusion_over_time, diffusion_tensor_over_time, eigenvalues_of_diffusion_tensor_over_time, eigenvectors_of_diffusion_tensor_over_time, unit_cell_matrix, energetic_disorder, temperature, path_to_crystal_file=None):
    ...
    diffusion_tensor_yx = []; diffusion_tensor_yy = []; diffusion_tensor_yz = []
    diffusion_tensor_zx = []; diffusion_tensor_zy = []; diffusion_tensor_zz = []
    ...
    # Eighth, obtain the visual of the direction of the diagonalised diffussion tensor. 
    diffusion_anisotrophy_over_time = Trajectory(path_to_place_data_in+'/'+'diffusion_anisotrophy.xyz',mode='w')
    if path_to_crystal_file is None:
        crystal_system = Atoms()
    else:
        crystal_system = read(path_to_crystal_file)
    for eigenvalues, eigenvectors in tqdm(zip(eigenvalues_of_diffusion_tensor_over_time, eigenvectors_of_diffusion_tensor_over_time), total=len(eigenvalues_of_diffusion_tensor_over_time)):
        ...
        diffusion_anisotrophy_over_time.write(eigenvectors_at_sampled_time)
    diffusion_anisotrophy_over_time.close()
    ...
    exciton_diffusion_over_time = Trajectory(path_to_place_data_in+'/'+'exciton_diffusion_over_time.xyz',mode='w')
    for time_index in trange(len(positions_at_time[0])):
        ...
        exciton_diffusion_over_time.write(excitons)
    exciton_diffusion_over_time.close()
    ...
    # Tenth, gather data for saving to excel.
