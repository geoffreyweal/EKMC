...
"""
import numpy as np
from EKMC.EKMC_Setup.get_neighbours_methods.get_rate_constant_data import get_rate_constant_data
...
def rate_constant_data_for_same_molecule_in_different_cells(kinetic_model, index, molecule, centre_of_mass, kinetics_details, electronic_coupling_data, all_local_neighbourhoods, all_cell_points, displacement_vectors):
	...
	for cell_point, displacement_vector in zip(all_cell_points, displacement_vectors):
		...
		all_local_neighbourhoods[index][index].append((cell_point, rate_constant_data1))
...
def rate_constant_data_for_different_molecules_in_origin_cell(kinetic_model, index1, index2, molecule1, molecule2, centre_of_mass1, centre_of_mass2, kinetics_details, electronic_coupling_data, all_local_neighbourhoods):
	...
	all_local_neighbourhoods[index1][index2].append((zero_point, rate_constant_data1))
	...
	all_local_neighbourhoods[index2][index1].append((zero_point, rate_constant_data2))
...
def rate_constant_data_for_different_molecules_in_different_cells(kinetic_model, index1, index2, molecule1, molecule2, centre_of_mass1, centre_of_mass2, kinetics_details, electronic_coupling_data, all_local_neighbourhoods, all_cell_points, displacement_vectors):
	...
	for cell_point, displacement_vector in zip(all_cell_points, displacement_vectors):
		...
		# Second, Check that this cell point and it's minus have not already been recorded.
		minus_cell_point = tuple(-uc for uc in cell_point)
		if any([(cell_point == neighbourhood[0]) for neighbourhood in all_local_neighbourhoods[index1][index2]]):
			raise Exception('huh?')
		if any([(minus_cell_point == neighbourhood[0]) for neighbourhood in all_local_neighbourhoods[index2][index1]]):
			raise Exception('huh?')

		# Third, get inputs for getting rate constant data. 
		# Note that the distance and electronic_coupling for mol1 -> mol2 are the same as for mol2 -> mol1
		electronic_coupling = electronic_coupling_data.get((index1+1, index2+1, *cell_point), None)

		# Fourth, we calculate the rate_constant_data1 and rate_constant_data2 as the kinetics_details for molecule 1 and molecule 2 may be different (for the process mol1 -> mol2 compared to the process mol2 -> mol1)
		# If mol 1 and mol2 have same attributes in kinetics_details, probably rate_constant_data1 == rate_constant_data2

		# 4.1: Focusing on mol1 -> mol2+cell_point. 
		...
		all_local_neighbourhoods[index1][index2].append((cell_point, rate_constant_data1))

		# 4.2: Focusing on mol2 -> mol1-cell_point
		...
		all_local_neighbourhoods[index2][index1].append((minus_cell_point, rate_constant_data2))
