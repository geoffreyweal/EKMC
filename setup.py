...
def find_scripts():
  ...
  for scripts_folder in scripts_folders:
    for root, dirs, files in os.walk('EKMC/'+scripts_folder, topdown=False):
      for file in files:
...
setup(name='EKMC',
      packages=find_packages(root='EKMC'),
      entry_points={'console_scripts': ['ekmc=EKMC.cli.main:main']},
      ...
      download_url = 'https://github.com/geoffreyweal/EKMC/archive/v'+str(get_version_number())+'.tar.gz',
      ...
      install_requires=['numpy', 'ase>=3.19.0', 'scipy', 'networkx', 'packaging', 'tqdm', 'xlsxwriter', 'SUMELF @ git+https://github.com/geoffreyweal/SUMELF.git'],
      classifiers=[
