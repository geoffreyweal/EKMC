...
def get_diffusion_tensor_over_time(times, displacement_vectors_from_initial_position, cpu_count=1):
    ...
    diffusion_tensor_over_time = process_map(get_diffusion_tensor_at_time, get_inputs(times, displacement_vectors_from_initial_position), max_workers=cpu_count, unit=' sample times', total=len(times))

    # Second, diagonalise the diffusion tensor over time
    diagonalised_diffusion_tensor_over_time = process_map(diagonalise_diffusion_tensor_at_time, diffusion_tensor_over_time, max_workers=cpu_count, unit=' sample times', total=len(times))
